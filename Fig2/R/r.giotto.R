...
t0 <- Sys.time()
suppressWarnings({
  giksim <- createGiottoObject(expression = as.matrix(regio_count),
    spatial_locs = coords,
...
suppressWarnings({
    gikprm <- createGiottoObject(expression = as.matrix(regio_perm),
      spatial_locs = coords,
      instructions = createGiottoInstructions(show_plot = FALSE, save_plot = FALSE))
...
t0 <- Sys.time()
suppressWarnings({
  girsim <- createGiottoObject(expression = as.matrix(regio_count),
    spatial_locs = coords,
    instructions = createGiottoInstructions(show_plot = FALSE, save_plot = FALSE))
...
suppressWarnings({
    girprm <- createGiottoObject(expression = as.matrix(regio_perm),
      spatial_locs = coords,
      instructions = createGiottoInstructions(show_plot = FALSE, save_plot = FALSE))
